...
class UAnomalyTransformer(BaseDeepAD):
    def __init__(self, exp_type = None, seq_len=100, stride=1, lr=0.0001, epochs=10, batch_size=32,
                 epoch_steps=20, prt_steps=1, device='cuda',
                 k=3, verbose=2, random_state=42):
        super(UAnomalyTransformer, self).__init__(
            model_name='UAnomalyTransformer', data_type='ts', epochs=epochs, batch_size=batch_size, lr=lr,
    ...
    def fit(self, X, y=None):
        ...
        dataloader = DataLoader(train_seqs, 
    ...
    def eval(self, X):
        ...
        with torch.no_grad():
            test_X_tensor = torch.tensor(seqs, dtype=torch.float32).to(self.device)
            pred_Y, series, prior, _  = self.net(test_X_tensor)
            pred_Y_np = pred_Y.cpu().numpy()
            
            if 'physical' in self.exp_type:
                train_error = np.abs(pred_Y_np - seqs)
                return train_error[:, -1, :].reshape(-1, self.output_dim)
            else:
                train_error = np.mean(np.square(pred_Y_np[:, -1, :] - seqs[:, -1, :]), axis = 1)
                return train_error.reshape(-1, 1)


    def decision_function(self, X, y_true, return_rep=False):
        seqs = get_sub_seqs(X, seq_len=self.seq_len, stride=1)
        ...
        with torch.no_grad():
            ...
            pred_Y, series, prior, _  = self.net(test_X_tensor)
            pred_Y_np = pred_Y.cpu().numpy()
            y_score = np.abs(pred_Y_np - seqs)

            if 'physical' in self.exp_type:
                y_score = np.abs(pred_Y_np - seqs)
                return y_score[:, -1, :].reshape(-1, 18), y_true[:y_score.shape[0]]

            elif 'cyber' in self.exp_type:
                y_score = np.mean(np.square(pred_Y_np[:, -1, :] - seqs[:, -1, :]), axis=1)
                return y_score.reshape(-1, 1), y_true[1:]
