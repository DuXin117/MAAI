function [HL,HV]=EnthalpyB(T,P,x,Mw)
%Computes the Enthalpies of the bottom stage 
%Inputs:
%T-->Temperature of feed
%P-->Pressure of feed
%x-->Vapor composition of feed
%Mw-->Molecular weight of components
%Outputs:
%HL-->Enthalpy liquid
%HV-->Enthalpy vapor
...
H1formfix= -74.9; %(KJ/mol) 
H2formfix= -84.738; %(KJ/mol) 
H3formfix= -103.89; %(KJ/mol) 
H4formfix= -126.19; %(KJ/mol)
...
Hg1f=((A1cpf*(T))+(B1cpf*(T^2)/2)+(C1cpf*(T^3)/3)-(A1cpf*(298.15))-(B1cpf*(298.15^2)/2)-(C1cpf*(298.15^3)/3))*(8.31445/(1000)); %KJ/mol
Hg2f=((A2cpf*(T))+(B2cpf*(T^2)/2)+(C2cpf*(T^3)/3)-(A2cpf*(298.15))-(B2cpf*(298.15^2)/2)-(C2cpf*(298.15^3)/3))*(8.31445/(1000)); %KJ/mol
Hg3f=((A3cpf*(T))+(B3cpf*(T^2)/2)+(C3cpf*(T^3)/3)-(A3cpf*(298.15))-(B3cpf*(298.15^2)/2)-(C3cpf*(298.15^3)/3))*(8.31445/(1000)); %KJ/mol
Hg4f=((A4cpf*(T))+(B4cpf*(T^2)/2)+(C4cpf*(T^3)/3)-(A4cpf*(298.15))-(B4cpf*(298.15^2)/2)-(C4cpf*(298.15^3)/3))*(8.31445/(1000)); %KJ/mol
...
HV1=0;
for i=1:k
  HV1=HV1+(HVTotal(i))*x(i)/1000; %Hv (KMJ/mol or 1000*KJ/mol)
end
